...
def main():
	...
	for order in orders:
		...
		csv_temp = dest_path + os.path.sep + 'temp.csv'
		csv_file = dest_path + os.path.sep + 'metadata.csv'
		try:
			with open(csv_temp, 'w', encoding = 'UTF-8') as csvfile:
				writer = csv.DictWriter(csvfile, fieldnames=columns)
				writer.writeheader()
				for data in metadatas:
					writer.writerow(data)
			with open(csv_temp, 'r', encoding = 'UTF-8') as csvtemp:
				with open(csv_file, 'w', encoding = 'UTF-8') as csvfile:
					csvfile.write(csvtemp.read().replace('\n\n','\n'))
		except IOError:
			print("I/O error")
